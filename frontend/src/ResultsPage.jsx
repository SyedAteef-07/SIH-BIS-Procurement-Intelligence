...
const tabs = [['recommended', 'Recommended Standard'], ['related', 'Related Standards'], ['status', 'Status & Amendments'], ['gaps', 'Gap Analysis'], ['certification', 'Certification']];
...
export default function ResultsPage({ result, initialQuery, onNewSearch }) {
  ...
  function downloadReport() {
    const lines = ['BIS Procurement Intelligence — Analysis', '', analyzedInput, '', ...recommendations.flatMap((s, i) => [`${i + 1}. ${code(s)} — ${s.title}`, relevanceLabel(s), `Status: ${statusLabel(s.status)}; Edition: ${s.edition || s.revision || 'Not provided'}`, ...strings(s.supporting_evidence), '']), presentationText(result?.explanation), 'Gap Analysis', presentationText(gapAnalysis?.scope), ...(gapAnalysis?.checks || []).flatMap(c => [c.label + ': ' + c.status, c.action, ...c.evidence, 'Metadata context: ' + c.source_evidence]), ...gaps, 'Related standards', ...relatedStandards.map(s => code(s) + ': ' + s.title + ' / ' + s.relationship_type + ' / ' + presentationText(s.relationship_note)), 'Certification', ...certifications.map(c => typeof c === 'string' ? c : [c.standard_code, c.name, c.authority, statusLabel(c.status), presentationText(c.note)].join(' / ')), 'AI-assisted recommendations should be verified against official BIS sources before final procurement use.'];
  ...
  return <div className="page-shell"><BrandHeader /><main className="results-main">
    ...
    <section id="result-panel" role="tabpanel" aria-labelledby={'tab-' + activeTab} className="result-panel">
      ...
      {activeTab === 'related' && <><h2>Related Standards</h2>{relatedStandards.length ? relatedStandards.map((s, i) => <StandardCard key={i} standard={s} />) : <EmptyState>No related standards found for this recommendation.</EmptyState>}</>}
